...
$property->appendChild($domDocument->createElement('status','vacant'));
...
if($domDocument->save('../multimedia/xml_file/feed.xml')){ // save as file
	$_SESSION['alert_xml'] = "<div class='alert alert-success'><button data-dismiss='alert' class='close' type='button'>x</button><strong>Well done!</strong> Dubizzle Feed uploaded.</div>";
}else{
	$_SESSION['alert_xml'] = "<div class='alert alert-error'><button data-dismiss='alert' class='close' type='button'>x;</button><strong>Oh snap!</strong> Dubizzle Feed not added.</div>";
}
