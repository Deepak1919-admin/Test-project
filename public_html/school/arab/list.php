...
include("banner.php");
?>
<div class="pro-head">شراء المباشر</div>
	<div class="container">
		<div class="list-con">
			<div class="col-sm-3">
				<div class="list-sec">
					<form name="sort_pro" id="sort_pro" method="GET">
						<div class="fil-head">تصفية حسب</div>
						<div class="listing-sec">
							<div class="listing-title">درجة</div>
							...
							<?php foreach($GetAllGrade as $grade){
								...
								?>
								<option value="<?php echo $grade['id']; ?>" <?php echo $selected;?>><?php echo $grade['title_arab']; ?></option>
							<?php } ?>
							</select>
						</div>
						<div class="listing-sec">
							<div class="listing-title">جنس</div>
							<select class="list-select" name="gender" id="gender">
								<option value="">حدد نوع الجنس</option>
								<option value="boy" <?php if($_GET['gender']=="boy"){ echo "selected";}?>>صبي</option>
								<option value="girl" <?php if($_GET['gender']=="girl"){ echo "selected";}?>>فتاة</option>
							</select>
						</div>
						<div class="listing-sec">
							<div class="listing-title">اسم العنصر</div>
							<select class="list-select" name="item" id="item">
								<option value="">حدد اسم المدينة</option>
							<?php foreach($GetAllItem as $item){
								...
								?>
								?>
								<option value="<?php echo $item['id']; ?>" <?php echo $select;?>><?php echo $item['title_arb']; ?></option>
							<?php } ?>
							</select>
						</div>
						
						
						<div class="list-sec-bt"><input type="submit" name="submit" value="ترسل أي"></div>
					</form>
				</div>
			</div>
			<div class="col-sm-9">
							...
							<div class="col-lg-3 col-md-4 col-xs-6 ls-item">
								<div class="list-sub-in">
									...
									<div class="inlist-sec">
										<div><a href="detail.php?pid=<?php echo $product[$i]['id']; ?>&mmid=5"><?php echo $product[$i]['title_arb']; ?></a></div>
										<div><!--AED <?php echo $product[$i]['price']; ?> |--> <a href="detail.php?pid=<?php echo $product[$i]['id']; ?>&mmid=5">اشتري الآن</a></div>
									</div>
								</div>
							</div>
				<?php 	}
					}else{
						echo '<div class="no-results"><p class="h2">لا توجد نتائج</p><p>يرجى محاولة تغيير بحثك وحاول مرة أخرى.</p></div>';
					}?>
				</div>
			</div>
			</div>
