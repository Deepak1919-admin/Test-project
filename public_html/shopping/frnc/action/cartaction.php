  ...
  if(!empty($idarray)) 
  {
	...
	{ 
	 ...
	 remove_from_cart($IDVal);
	  ...
	  { 
	  	$chkstock=$objPdtPrice ->GetRowContent($IDVal);


	  	if($chkstock['stock']>$_POST['quantity'.$IDVal]){ 
			  $_SESSION['cart'][$IDVal]['quantity']=$_POST['quantity'.$IDVal];
			  unset($_SESSION['stockalert']);
		}else{
			$_SESSION['stockalert']=$IDVal;
		}
	  }
    }
	  header('location:../cart.php');
