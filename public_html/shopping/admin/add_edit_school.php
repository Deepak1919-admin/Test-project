...
if (isset($_POST['submit'])) 
{
	...
	if (!empty($_FILES['slogo'])) {
		...
		$temp_name = $_FILES['slogo']['tmp_name'];
		$file_name = $_FILES['slogo']['name'];
		$file_type = $_FILES['slogo']['type'];
		$file_size = $_FILES['slogo']['size'];
		$result = $_FILES['slogo']['error'];

		$real_name = strtolower($_FILES['slogo']['name']);
		...
		if (($temp_name == "")) {
			...
			// generate thumbnail
			$img->GenerateThumbFile($temp_name, $upload_dir . $up_file_name);
			$_POST['school_logo'] = $up_file_name;
		}
	}
        ...
        <div class="row">
            ...
            <div class="col-lg-12">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="file" class="" id="typeahead1" name="slogo">(150 * 150)
					</div>
				</div>
				...
				<?php
                          if (!empty($row['school_logo'])) {?>
                                    <div class="control-group">
                                        <label class="control-label" for="fileInput">School Logo</label>
                                        <div class="controls">
                                            <div style="margin-left:175px;right: 70px; top: 100px;"><img src="<?php echo "../multimedia/school/" . $row['school_logo'] ?>"></div>
                                        </div>
                                    </div>
<?php }
                                if (!empty($row['banner'])) {?>
