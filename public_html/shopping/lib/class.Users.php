...
$arrData["code"]=clean_string($arrData["code"]);

	if($arrData["code"]!="")
	{
	    $que="SELECT " .TBL_USERS. ".id," .TBL_USERS. ".first_name," .TBL_USERS. ".email FROM " .TBL_USERS. " INNER JOIN " .TBL_SCHOOL. " WHERE ".TBL_USERS.".email='".$arrData["email"]."' and ".TBL_SCHOOL.".code='".$arrData["code"]."' AND ".TBL_USERS.".password='".$arrData["password"]."' AND ".TBL_SCHOOL.".status='1'";
	}
	else{

	 $que="SELECT * FROM " .TBL_USERS. " WHERE email='".$arrData["email"]."' AND status=1 AND password='".$arrData["password"]."'"; 
	}
        $rs = $db->query($que);
        if($rs->num_rows()>0){
            $row = $rs->fetch_assoc();
	    ...
	    $_SESSION['dream']['code']=$arrData["code"];
